...
export interface IAdditionsService {
  ...
  ): Promise<ApiResponse<IPagingData<IProjectAdditionList>>>;
  getFundsList(
    filters: IProjectAdditionFilters
  ...
  updateAdditionWithMov(
    id: number,
    addition: IAdditionsWithMovements
  ): Promise<ApiResponse<IAdditionsWithMovements>>;
  executeUpdateAdditionWithMov(
    id: number,
    addition: IAdditionsWithMovements
  ): Promise<ApiResponse<IAdditionsWithMovements>>;
...
export default class AdditionsService implements IAdditionsService {
  ...
  async createAdditions(
    addition: IAdditionsWithMovements
  ): Promise<ApiResponse<IAdditionsWithMovements | any>> {
    ...
    const { bandEquals, bandZeros, income, spend } =
      await this.totalsMovementsValidations(addition);
    ...
    if (bandControl) {
      const arrayResponse = `@@@Se ha encontrado un error en los datos, revisa rutas presupuestales@@@NOEXISTERUTASPRESUPUESTARIAS@@@${JSON.stringify(
  ...
  async getProjectsList(
    ...
    const toSend = await this.completeProyectsList(projects.array);
  ...
  async completeProyectsList(
    projectsVinculationList: IProjectsVinculation[]
  ): Promise<IProjectAdditionList[]> {
    // 1. buscar proyectos funcionales
    // const projectOperationList =
    //   await ProjectOperationRepository.getProyectsByIds(
    //     projectsVinculationList
    //       .filter((p) => p.type === "Funcionamiento")
    //       .map((i) => i.id)
    //   );
    ...
    projectsVinculationList.forEach((p) => {
      //...buscamos lista correspondiente
      //       toSend.push({
      //         ...p,
      // //         assignmentValue: //...buscamos lista correspondiente
      // // budgetValue  //...buscamos lista correspondiente
      // // conceptProject  //...buscamos lista correspondiente
      // // projectId //...buscamos lista correspondiente
      //       });
    });
  ...
  async namesAddtionsValidations(
    ...
    const nameActAdminDis: string = addition
      .headAdditon!.actAdminDistrict.trim()
      .toUpperCase();
    const nameActAdminSap: string = addition
      .headAdditon!.actAdminSapiencia.trim()
      .toUpperCase();
  ...
  async executeCreateAdditions(
    ...
    const add = await this.additionsRepository.createAdditions({
      actAdminDistrict: addition
        .headAdditon!.actAdminDistrict.trim()
        .toUpperCase(),
      actAdminSapiencia: addition
        .headAdditon!.actAdminSapiencia.trim()
        .toUpperCase(),
  ...
  async executeUpdateAdditionWithMov(
    id: number,
    addition: IAdditionsWithMovements
  ): Promise<ApiResponse<IAdditionsWithMovements>> {
