...
export default class PacService implements IPacService {
  ...
  uploadPac = async (file: any, body: IBody): Promise<ApiResponse<any>> => {
    ...
    const { validTemplateStatus,
      ...
      if(rowsWithFieldsEmpty.length>0){
        return new ApiResponse({responseSave:false, erros:rowsWithFieldsEmpty}, EResponseCodes.OK, "Algún dato de la ruta está vacío");
    ...
    switch (typePac) {
      ...
      case 'Nueva versión':
        ...
        await this.pacRepository.inactivateVersionPac(versionFixed,pacsByExerciseFixed)
        break;
      default:
        break;
    }
  ...
  async searchAnnualDataRoutes(data: IPacAnnualAdapter): Promise<ApiResponse<IPagingData<IPacPrimary> | IResultSearchAnnualizationByRoute | IPacFilters | null>> {
    ...
    return new ApiResponse(objResult, EResponseCodes.OK, "Obteniendo las anualizaciones");
