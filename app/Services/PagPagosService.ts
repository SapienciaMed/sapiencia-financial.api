      ...
      if (items && Array.isArray(items) && items.length > 0) {
        for (const item of items) {
          ...
          if (rprRegistroPresupuestal) {
            const rprCodigo = rprRegistroPresupuestal?.$attributes?.id;
            const vinculacionRprIcd = await LinkRpcdp.query()
              .select('VRP_CODRPR_REGISTRO_PRESUPUESTAL')
            ...
            if (vinculacionRprIcd) {
              ...
              console.error(`No se encontró una coincidencia para el item con posición ${item.posicion} y vinculaciónRpCode ${item.vinculacionRpCode}`);
            }
          }
