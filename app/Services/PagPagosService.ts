...
export default class PagoService implements IPagoService {
  ...
  public readExcel = async (fileBuffer: Buffer, _usuarioCreo:any): Promise<ApiResponse<IResponseUploadMasive | null>> => {
  ...
  private async insertItemsToDatabase(items: any[], _validator: any, model: any): Promise<void> {
    for (const item of items) {
