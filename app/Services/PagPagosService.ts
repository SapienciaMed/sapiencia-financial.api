...
import {
  ...
  IPagoResponse,
  IPagoMasiveSave,
} from "App/Interfaces/PagPagosInterfaces";
...
import {
  ...
  IErrorsUploadMasive,
} from "App/Interfaces/UploadMasiveInterfaces";
...
export default class PagoService implements IPagoService {
    ...
    if (Object.keys(result).length > 0) {
      ...
      if (items && Array.isArray(items) && items.length > 0) {
        for (const item of items) {
          ...
          responseData = await this.insertItemsToDatabase(
            [pago],
            PagPagosValidator,
            PagPagos
          );
        ...
        console.error(
          'La propiedad "items" no es un arreglo válido o está vacía.'
        );
      }
    } else {
      console.error(
        'La estructura de datos "result" está vacía o no es válida.'
      );
    }
  ...
  async processBase64(
    fileContent: string,
    mes: number
  ): Promise<ApiResponse<IResponseUploadMasive | null>> {
