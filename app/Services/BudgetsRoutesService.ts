...
export interface IBudgetsRoutesService {
    ...
    getPospreByProjectAndFund(projectId:number,fundId:number): Promise<ApiResponse<IPosPreSapiencia[] | null>>;
}
...
export default class BudgetsRoutesService implements IBudgetsRoutesService {
    ...
    async getPospreByProjectAndFund(projectId: number, fundId: number): Promise<ApiResponse<IPosPreSapiencia[] | null>> {
        const res = await this.BudgetsRoutesRepository.getPospreByProjectAndFund(projectId, fundId);

        let posrpreList:IPosPreSapiencia[] = []
        
        res?.forEach((pospreSapiencia:any)=>{
            let pospreExtract = pospreSapiencia.$preloaded?.pospreSapiencia?.$attributes
