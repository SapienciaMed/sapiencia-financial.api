...
export default class FundsUploadMasiveService
  implements IFundsUploadMasiveService
{
  ...
  > => {
    ...
    if (validTitles !== null)
      return new ApiResponse(
        this.arrayErrorStructure,
        EResponseCodes.FAIL,
        "El archivo no cumple la estructura"
      );
    if (this.arrayEmptyData.length > 0 || bandControlForValidEmpty)
      return new ApiResponse(
        this.arrayEmptyData,
        EResponseCodes.FAIL,
        "Algún dato está vacío"
      );
    if (this.arrayErrorDates.length > 0 || bandControlForValidDates)
      return new ApiResponse(
        this.arrayErrorDates,
        EResponseCodes.FAIL,
        "Error en fechas"
      );
    if (this.arrayErrorExist.length > 0 || bandControlForExistFunds)
      return new ApiResponse(
        this.arrayErrorExist,
        EResponseCodes.FAIL,
        "El fondo ya existe"
      );
    if (this.arrayErrorRepeat.length > 0 || bandControlForRepeatExcelFunds)
      return new ApiResponse(
        this.arrayErrorRepeat,
        EResponseCodes.FAIL,
        "Tiene datos duplicados en el archivo"
      );
  ...
  validTitles(
    headers: string[]
  ): IResponseUploadMasive | IErrorsUploadMasive[] | null {
