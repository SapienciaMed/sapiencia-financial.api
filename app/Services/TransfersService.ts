...
export interface ITransfersService {
  ...
  ): Promise<ApiResponse<IPagingData<ITransfers>>>;
  createTransfers(
    transfer: ITransfersWithMovements
  ): Promise<ApiResponse<ITransfersWithMovements>>;
  executeCreateTransfers(
    transfer: ITransfersWithMovements
  ): Promise<ApiResponse<ITransfersWithMovements | any>>;
  totalTransferCalculator(
    transfer: ITransfersWithMovements
  ): Promise<ApiResponse<number>>;
  getTransferById(id: number): Promise<ApiResponse<ITransfersWithMovements>>;
  getFundsList(
    filters: IProjectTransferFilters
  ): Promise<ApiResponse<IPagingData<IFundsTransferList>>>;
  getPosPreList(): Promise<IPosPreTransfer | string[]>;
  getPosPreSapienciaList(
    filters: IProjectTransferFilters
  ): Promise<ApiResponse<IPagingData<IPosPreSapienciaTransferList>>>;
  updateTransferWithMov(
    id: number,
    transfer: ITransfersWithMovements
  ): Promise<ApiResponse<ITransfersWithMovements | any>>;
  executeUpdateTransferWithMov(
    id: number,
    transfer: ITransfersWithMovements
  ): Promise<ApiResponse<ITransfersWithMovements | any>>;

  //*Validaciones Front y CREAR/ACTUALIZAR
  namesAndObservationsValidations(
    transfer: ITransfersWithMovements
  ): Promise<Boolean>;
  totalsMovementsValidations(
    transfer: ITransfersWithMovements
  ): Promise<string>;
...
export default class TransfersService implements ITransfersService {
  ...
  async createTransfers(
    ...
    for (let i of transfer.transferMovesGroups) {
      for (let j of i.data) {
        ...
        const resp = await this.budgetPathValidations(
          idCard,
          projectId,
          foundId,
          posPreId
    ...
    if (bandControl) {
      const arrayResponse = `@@@Se ha encontrado un error en los datos de inserción, revisa rutas presupuestales@@@NOEXISTERUTASPRESUPUESTARIAS@@@${JSON.stringify(
        arrayCardsErrorRoutes
      )}@@@Se ha encontrado un error en los datos, revise proyectos@@@PROYECTOS@@@${JSON.stringify(
        arrayCardsErrorProjects
      )}@@@Se ha encontrado un error, datos duplicados en el sistema@@@RUTASPRESUPUESTARIASREPETIDAS@@@${JSON.stringify(
        arrayCardsErrorRepeatRoutes
      )}`;
    ...
    for (let i of transfer.transferMovesGroups) {
      ...
      if (resultPiToPf === "true") {
        ...
        errorType =
          "Se esta intentado trasladar de un Proyecto de Inversión a uno de Funcionamiento";
  ...
  async updateTransferWithMov(
    ...
    if (bandControl) {
      const arrayResponse = `@@@Se ha encontrado un error en los datos de actualización, revisa rutas presupuestales@@@NOEXISTERUTASPRESUPUESTARIAS@@@${JSON.stringify(
        arrayCardsErrorRoutes
      )}@@@Se ha encontrado un error en los datos, revise proyectos@@@PROYECTOS@@@${JSON.stringify(
        arrayCardsErrorProjects
      )}@@@Se ha encontrado un error, datos duplicados en el sistema@@@RUTASPRESUPUESTARIASREPETIDAS@@@${JSON.stringify(
  ...
  async getPosPreSapienciaList(
    filters: IProjectTransferFilters
  ): Promise<ApiResponse<IPagingData<IPosPreSapienciaTransferList>>> {
    const posPreRes = await this.pospreSapRepository.getPosPreSapienciaList(
      filters
    );
  ...
  async namesAndObservationsValidations(
    transfer: ITransfersWithMovements
  ): Promise<Boolean> {
    const nameActAdminDis: string = transfer
      .headTransfer!.actAdminDistrict.trim()
      .toUpperCase();
    const nameActAdminSap: string = transfer
      .headTransfer!.actAdminSapiencia.trim()
      .toUpperCase();
    const nameObservations: string = transfer
      .headTransfer!.observations.trim()
      .toUpperCase();
  ...
  async totalsMovementsValidations(
    transfer: ITransfersWithMovements
  ): Promise<string> {
  ...
  async budgetPathValidations(
    ...
    const resultRPP = await this.budgetRouteRepository.getBudgetForAdditions(
      projectId,
      foundId,
      posPreOriginId,
      posPreId
  ...
  async executeCreateTransfers(
    ...
    const totalTransfer: ApiResponse<number> =
      await this.totalTransferCalculator(transfer);

    if (!totalTransfer || totalTransfer.data <= 0) {
      return new ApiResponse(
        null,
        EResponseCodes.FAIL,
        "Ocurrio un error al calcular el total del traslado. Copie los datos de trabajo y consulte con el admin del sistema"
      );
    }

    //* Agregar cabecera de traslado
    const addToHead = await this.transfersRepository.createTransfers({
      actAdminDistrict: transfer
        .headTransfer!.actAdminDistrict.trim()
        .toUpperCase(),
      actAdminSapiencia: transfer
        .headTransfer!.actAdminSapiencia.trim()
        .toUpperCase(),
  ...
  async executeUpdateTransferWithMov(
    id: number,
    transfer: ITransfersWithMovements
  ): Promise<ApiResponse<ITransfersWithMovements | any>> {
    //* Borramos los que encontremos viejos
    const deleteMovementsOld =
      await this.movementsRepository.deleteMovementById(id);
  ...
  async noPermitedInversionProyToFunctionalProy(
    i: ITransfersWithMovements | any
    ...
    for (const iter of i.data) {
      ...
      // console.log('akive')
      const resultProj = await this.projectRepository.getProjectById(
        iter.projectId
      );
      // console.log('akive2',resultProj)
      const referenceCodeProject = resultProj?.projectId.toString();

      if (iter.type === "Origen" && referenceCodeProject !== "9000000")
        bandInveProject = true;
      if (iter.type === "Destino" && referenceCodeProject === "9000000")
        bandFuncProject = true;
    }
