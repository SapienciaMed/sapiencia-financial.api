...
export interface IFunctionalAreaRepository {
  ...
  ): Promise<IFunctionalArea>;
  updateFunctionalArea(
    functionalArea: IFunctionalArea,
    id: number
  ): Promise<IFunctionalArea | null>;
  createProjectFunctionalArea(
    projectsVinculate: IProjectsVinculate
  ): Promise<IProjectsVinculation[]>;
  updateProjectFunctionalArea(
    projectsVinculate: IProjectsVinculate
  ): Promise<IProjectsVinculation[] | null>;
  ...
  implements IFunctionalAreaRepository
{
  ...
  async createFunctionalArea(
    ...
    if (functionalArea.userCreate)
      toCreateFunctionalArea.userCreate = functionalArea.userCreate;
  ...
  async createProjectFunctionalArea(
    ...
    await Promise.all(
      projectsVinculate.projects.map(async (project) => {
        ...
        if (projectsVinculate.userCreate)
          toCreate.userCreate = projectsVinculate.userCreate;
        await toCreate.save();
  ...
  async updateProjectFunctionalArea(
    ...
    await Promise.all(
      projectsVinculate.projects.map(async (project) => {
        const searchProjectsVinculation = await ProjectsVinculation.find(
          project.id
        );
    ...
    await Promise.all(
      vinculations.map(async (project) => {
        await project.save();
  ...
  async deleteProjectFunctionalArea(
    ...
    try {
      await ProjectsVinculation.query().where("id", projectVinculate).delete();
      return true;
    } catch {
      return false;
    }
  ...
  async getFunctionalAreaByNumber(
    number: string
  ): Promise<IPagingData<IFunctionalArea>> {
